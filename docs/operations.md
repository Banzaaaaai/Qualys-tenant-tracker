...
aggregator -- release-note lookups use official Qualys sources. The optional external heartbeat contacts the configured monitoring endpoint.
...
[`docs/sample-email-change-notification.html`](sample-email-change-notification.html).
The tables use plain inline styles (no external CSS, no flexbox/grid)
specifically so they render correctly in Outlook.

**Email delivery now uses a persistent outbox.** Failed baseline, forced, change, and public-release messages retry on subsequent checks. See [reliability and recovery](reliability.md) for delivery guarantees and recovery procedures.
...
`notifications` counts by message type and an unsuccessful run status,
...
Failed messages remain in the durable outbox. The next check retries the original notification, including its original change details; no forced inventory email is needed. Already queued messages may also be retried during an API-failure run.
